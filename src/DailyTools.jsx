...
function NumberCheck({ lang }) {
  ...
  return (
    <article className="dailyCard businessCard" id="number-check">
      <div className="dailyCardHead">
        ...
        <div>
          ...
          <h3>
            {lang === "ja"
              ? "法人・インボイス番号チェック"
              : "Corporate & invoice number check"}
          </h3>
        </div>
      </div>
      <label className="stackField">
        ...
        <input
          value={value}
          onChange={(e) => setValue(cleanNumber(e.target.value).slice(0, 14))}
      ...
      <div className={`numberStatus ${status}`}>
        ...
        {lang === "ja" ? messages[status][0] : messages[status][1]}
      </div>
      <p className="toolNote">
        {lang === "ja"
          ? "形式確認だけでは実在や登録状態を証明できません。国税庁で最終確認してください。"
          : "Format alone does not prove existence or registration. Confirm with the NTA."}
      </p>
      <div className="officialLinks">
        <a
          ...
          rel="noreferrer"
        >
          {lang === "ja" ? "法人番号公表サイト" : "Corporate registry"}
          <ExternalLink />
        </a>
        <a
          href="https://www.invoice-kohyo.nta.go.jp/index.html"
          target="_blank"
          rel="noreferrer"
        >
          {lang === "ja" ? "インボイス公表サイト" : "Invoice registry"}
          <ExternalLink />
        </a>
      </div>
      <ToolPulse
